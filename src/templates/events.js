...
function Event({event}){
    return (<div>
        ...
        <h4>{event.teacher}</h4>
        <p>{event.day} {new Date(event.start).toLocaleTimeString('sv-SE', {hour: '2-digit', minute:'2-digit', hour12: false})} - {new Date(event.end).toLocaleTimeString('sv-SE', {hour: '2-digit', minute:'2-digit', hour12: false})}</p>
    </div>);
    }
...
export default class Events extends React.Component
{
  ...
  appendData = (events) => events.map(x => ({...x, day: days[(new Date(x.start).getDay()+6) % 7] }));
      ...
      <div>
        {Object.keys(this.state.filter).map((f) => 
         Object.keys(this.state.filter[f]).map((k) => 
          <button type="button" onClick={(e) => this.toggleFilter(e,f,k)}key={k}>{k} - {this.state.filter[f][k] ? 'yes':'no'}</button>
        ))}
